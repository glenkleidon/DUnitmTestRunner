...
Function GetExistingPathsFromList(AList: string; ARootPath: string): string;
var
  lList: TStringlist;
  i: integer;
  lPath: string;
begin
  ...
  try
    lList.Text := StringReplace(AList, ';', #13#10, [rfReplaceAll]);
    for i := lList.Count - 1 downto 0 do
    begin
      lPath := stringReplace(lList[i],'"','',[rfReplaceAll]);
      if copy(lPath,1,1)='.' then
        lPath := ExpandFileName(includeTrailingPathDelimiter(ARootPath)+lPath);
      // Do we need to expand it first?
      if pos('$(',lPath)>0 then continue;
      if (length(lPath)=0) or (not DirectoryExists(lPath)) then
        lList.Delete(i);
    end;
    Result := StringReplace(lList.Text,#13#10,';',[rfReplaceAll]);
    result :=copy(Result,1,length(Result)-1); // remove trailing ;
  finally
    freeandnil(lList);
  end;
...
begin
  ...
  AProject.UnitSearchPath := GetExistingPathsFromList(AProject.UnitSearchPath,
    AProject.ProjectDir);
  AProject.IncludeSearchPath := GetExistingPathsFromList(AProject.IncludeSearchPath,
    AProject.ProjectDir);
  AProject.ResourceSearchPath := GetExistingPathsFromList(AProject.ResourceSearchPath,
    AProject.ProjectDir);
  AProject.ObjectSearchPath := GetExistingPathsFromList(AProject.ObjectSearchPath,
    AProject.ProjectDir);
